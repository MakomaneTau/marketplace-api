...
const USER="123e4567-e89b-42d3-a456-426614174000", PRODUCT="223e4567-e89b-42d3-a456-426614174000", CONVERSATION="323e4567-e89b-42d3-a456-426614174000", NOTIFICATION="423e4567-e89b-42d3-a456-426614174000", ORDER="523e4567-e89b-42d3-a456-426614174000";
...
it("starts an order delivery conversation",async()=>{auth();service.startOrderConversation.mockResolvedValue({id:CONVERSATION});const r=await request(app).post(`/api/v1/conversations/orders/${ORDER}`).set("Authorization","Bearer token");expect(r.status).toBe(201);expect(service.startOrderConversation).toHaveBeenCalledWith(USER,ORDER);});
