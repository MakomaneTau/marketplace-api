...
do $$ declare c uuid; m uuid; n integer; notification_href text; begin
...
select count(*), min(href) into n, notification_href from public.notifications where user_id='91000000-0000-4000-8000-000000000001' and type='message';
if m is null or n<>1 then raise exception 'MESSAGE_NOTIFICATION_FAILED'; end if;
if notification_href <> ('/seller/messages?conversation=' || c::text) then raise exception 'SELLER_NOTIFICATION_HREF_FAILED'; end if;
insert into public.user_preferences(user_id,notify_new_messages) values('91000000-0000-4000-8000-000000000001',false);
perform public.send_marketplace_message(c,'91000000-0000-4000-8000-000000000002','This should not notify the seller.');
select count(*) into n from public.notifications where user_id='91000000-0000-4000-8000-000000000001' and type='message';
if n<>1 then raise exception 'MESSAGE_NOTIFICATION_PREFERENCE_FAILED'; end if;
raise notice 'messaging smoke passed: idempotent conversation, participant message, routed notification, preference suppression'; end $$;
rollback;
